...
BEGIN
  ...
  ELSE
    IF to_jsonb(NEW) ? 'case_ref' AND (NEW.case_ref IS NULL OR trim(NEW.case_ref) = '') THEN NEW.case_ref := NEW.case_reference; END IF;
    IF to_jsonb(NEW) ? 'resident_status' AND (NEW.resident_status IS NULL OR trim(NEW.resident_status) = '') THEN NEW.resident_status := 'recibido'; END IF;
    RETURN NEW;
  END IF;
  FOR i IN 1..12 LOOP
...
-- --- 20260423120000 gemba_round_templates ---
...
CREATE INDEX IF NOT EXISTS gemba_round_templates_building_id_idx ON public.gemba_round_templates (building_id);
CREATE INDEX IF NOT EXISTS gemba_round_templates_active_idx ON public.gemba_round_templates (building_id, is_active);
...
CREATE POLICY "gemba_round_templates_select_all" ON public.gemba_round_templates FOR SELECT TO anon, authenticated USING (true);
DROP POLICY IF EXISTS "gemba_round_templates_insert_all" ON public.gemba_round_templates;
CREATE POLICY "gemba_round_templates_insert_all" ON public.gemba_round_templates FOR INSERT TO anon, authenticated WITH CHECK (true);
DROP POLICY IF EXISTS "gemba_round_templates_update_all" ON public.gemba_round_templates;
CREATE POLICY "gemba_round_templates_update_all" ON public.gemba_round_templates FOR UPDATE TO anon, authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "gemba_round_templates_delete_all" ON public.gemba_round_templates;
CREATE POLICY "gemba_round_templates_delete_all" ON public.gemba_round_templates FOR DELETE TO anon, authenticated USING (true);
