...
| Consejeria / Hallazgo | Ubicacion in situ | `Ubicacion in situ` dropdown does not make sense for the selected inspection point. | Use master data tied to the inspection point to populate valid location options. | P1 | Pending | Partial 2026-05-01 | Point-triggered hallazgos carry the selected Punto de Inspeccion and suggested zone/map metadata; full per-point location narrowing still needs richer master data. |
...
| Mapa | Map base view | Map itself looks good visually, but no pins or points are visible. | Display relevant route points, inspection points, issue markers, or other operational map markers. | P1 | Pending | Partial 2026-05-01 | When no saved waypoints exist, the map now renders route preview markers from the site GeoJSON; persisted issue markers still depend on the Supabase findings map view. |
| Mapa | Mark issue flow | `Marcar hallazgo` allows selecting a map point, but the map/form context only says `Ruta de recorrido`. | Make the selected point meaningful in the issue flow, with clear location context from the map selection. | P1 | Pending | Partial 2026-05-01 | Map-click hallazgos pass lat/lng and selected zone into Gemba metadata; remaining polish is showing that context more explicitly in the map popup and saved registry. |
...
| Mapa | New issue visibility | After saving an issue and returning to map, the issue point is not visible. | Show newly flagged issues as markers on the map after save. | P1 | Pending | Partial 2026-05-01 | Map has visible route fallback markers and still fetches saved Gemba findings from `v_inspection_findings_map`; immediate marker reflection after insert needs browser verification and may depend on the view extracting metadata lat/lng. |
...
| Supervision / Recorridos | Recorrido master data | Daily concierge recorridos, daily administradora de planta recorridos, and weekly supervisor recorridos need one definition source. | Maintain recorridos as master data/config with role, frequency, owner, and inspection points. | P1 | Pending | Partial 2026-05-01 | Gemba templates and rounds now carry `plan_role`/`plan_owner`; fallback plans split conserje daily plans from supervisor weekly plans. Persistent migrated templates may still need backfilled role metadata. |
| Supervision / Recorridos | Summary clarity | `Abierto`, `Atrasado`, and `Completado` appear to summarize conserje completions, but role scope is unclear. | Clarify whether summaries show all recorridos, only assigned role recorridos, or team oversight metrics. | P1 | Pending | Partial 2026-05-01 | Active and next-action cards now display `Dueño`; KPI labels still need a scope wording pass. |
| Supervision / Recorridos | Continue recorrido ambiguity | `Siguiente paso`, `Recorrido matutino`, and `Continuar puntos` make it unclear whether supervisor is continuing a conserje recorrido or their own recorrido. | Make recorrido ownership and action explicit before continuing. | P1 | Pending | Partial 2026-05-01 | Active/continue cards display owner role, and conserje only sees conserje-owned rounds; supervisor still has team oversight visibility. |
...
| Supervision / New work order | Supabase 400 error | Creating a work assignment errors with `Supabase 400 on work assignment`. | Debug and fix work assignment creation, likely tied to required controlled fields/foreign keys. | P1 | Pending | Partial 2026-05-01 | New work assignment payloads now include controlled area/assignee/priority plus `building_id`; no production write was performed, so save success still needs local/prod-safe verification. |
| Supervision / Projects and Actions | Existing project updates | No clear way to update existing projects/actions. | Add or expose update flow for existing work orders/projects/actions. | P1 | Pending | Partial 2026-05-01 | Supervisor/Gerencia work cards now expose an update modal for status, priority, due date, and progress comment stored in metadata history. |
...
| Supervision / Inventario | Reportar novedad | `Reportar novedad` opens the same fields and does not match supervisor responsibilities. | Replace supervisor inventory actions with purchase-order/status/comment workflows for stockouts and low-stock items. | P1 | Pending | Partial 2026-05-01 | Supervisor now has `Actualizar reposición` with status, OC reference, and comment. The legacy incident novelty path remains available for actual incidents. |
| Supervision / Inventario | Stockout actioning | Supervisor should be able to act on low stock/stockout flags. | Add actions for purchase order raised, comments, status updates, and follow-up ownership. | P1 | Pending | Partial 2026-05-01 | Procurement/status/comment workflow added for alert items; persistence uses `inventory_movements.metadata` and needs browser verification before marking done. |
