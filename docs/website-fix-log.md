...
| Consejeria / Recorridos | Status summary buttons | `En curso`, `Conteo operativo`, and `Foto de hallazgo` look like clickable controls but cannot be clicked. | Enable drill-down from each status summary into the relevant filtered detail view, or restyle them so they do not imply interaction. | P2 | Pending | Captured | Each summary either drills into useful detail or no longer appears clickable. |
...
| Consejeria / Nueva ejecucion | Area de base | `Area de base` is manually selected even though it should come from the selected master plan. | Pull `Area de base` from the selected plan maestro and avoid asking the conserje to choose it manually. | P1 | Pending | Wave 2 partial | Area options now come from the shared master-data catalog and selected Plan Maestro applies area automatically; hiding/locking after selection remains Wave 4 UX. |
| Consejeria / Nueva ejecucion | Frecuencia | `Frecuencia` is manually selected every execution even though it should come from the master inspection plan. | Pull frequency from the selected master inspection plan instead of asking the conserje each time. | P1 | Pending | Wave 2 partial | Frequency options now come from the shared inspection-plan taxonomy and selected Plan Maestro applies frequency automatically; hiding/locking after selection remains Wave 4 UX. |
...
| Consejeria / Hallazgo | Ubicacion in situ | `Ubicacion in situ` dropdown does not make sense for the selected inspection point. | Use master data tied to the inspection point to populate valid location options. | P1 | Pending | Captured | Location dropdown only shows options that make sense for the selected punto de inspeccion. |
| Consejeria / Hallazgo | Seguimiento options | Follow-up options need a practical operational list. | Research and define follow-up actions such as fixed, needs attention, escalate to junta, replace/buy part, or close as dealt with. | P2 | Pending | Wave 2 partial | Shared status/priority/escalation taxonomy is defined in config; applying richer follow-up actions in the hallazgo UI remains a later workflow pass. |
...
| Inventario | Registrar conteo interaction | Clicking `Registrar conteo` appears to do nothing because the form opens at the bottom of the screen. | Open the count registration form immediately in the user's current view, optimized for phone use. | P1 | Pending | Captured | On mobile, tapping `Registrar conteo` shows the form right away without requiring scrolling/searching. |
| Inventario | List placement | Operational lists can push the primary action form too far down. | Make list sections collapsible and place them after the operational action flow. | P1 | Pending | Captured | Conserje sees the count action flow first; long lists are collapsed and available lower on the page. |
| Inventario | Count form fields | Count form supports article, storage location, quantity/update, but lacks comments. | Keep article/location/quantity fields and add a comments field. | P2 | Pending | Captured | Staff can register a count with article, location, quantity/update, and optional comments. |
| Mapa | Map base view | Map itself looks good visually, but no pins or points are visible. | Display relevant route points, inspection points, issue markers, or other operational map markers. | P1 | Pending | Captured | Map shows useful points/markers without the user needing to guess where things are. |
| Mapa | Mark issue flow | `Marcar hallazgo` allows selecting a map point, but the map/form context only says `Ruta de recorrido`. | Make the selected point meaningful in the issue flow, with clear location context from the map selection. | P1 | Pending | Captured | Staff can select where an issue is on the map and see that location reflected in the form. |
| Mapa | Issue form location | Location dropdown has only a few unrelated items and does not match the selected map area or operational context. | Drive location options from master data and/or selected map point. | P1 | Pending | Captured | Location choices make sense for the selected point/area. |
| Mapa | Issue form segment | Segment dropdown does not make sense for the selected map point or issue. | Drive segment options from route/inspection master data or remove segment if not operationally useful. | P1 | Pending | Captured | Segment field is relevant to the selected map location, or the field is removed. |
...
| Mapa | New issue visibility | After saving an issue and returning to map, the issue point is not visible. | Show newly flagged issues as markers on the map after save. | P1 | Pending | Captured | Saved issues appear on the map immediately or after refresh. |
| Mapa | Map interaction model | Current map is not functional enough; click/drag or direct point flagging may be needed. | Define a practical mobile-first way for staff to flag the issue location on the map. | P2 | Pending | Needs design decision | Staff can easily mark where the issue is without confusing fields or hidden behavior. |
| Incidencias | Concierge visibility | Concierge view appears to show previous incidents broadly. | Show only incidents raised by the concierge or assigned to that concierge. | P1 | Pending | Captured | Concierge sees only their own raised incidents and items assigned to them. |
| Incidencias | Incident action | Concierge needs an action path for assigned incidents/work orders. | Allow assigned incidents/work orders to be marked closed with a comment and/or evidence. | P1 | Pending | Captured | Concierge can close an assigned item with completion notes and optional evidence. |
...
| Consejeria / Main screen | Welcome personalization | `Bienvenido Personal Conserjeria` is acceptable for now, but should later use the logged-in person's name. | After login is fixed, show `Bienvenido` plus the logged-in person's name. | P2 | Pending | Captured | Logged-in concierge sees a personalized welcome message. |
| Supervision / Role model | Future role split | Current `supervision` login will eventually split into administration/supervision responsibilities. | Keep current audit under supervision, but design fixes so administration/supervision role boundaries can be separated later. | P2 | Pending | Captured | Workflow changes do not hard-code a role model that blocks future administration split. |
| Supervision / Recorridos | Recorrido master data | Daily concierge recorridos, daily administradora de planta recorridos, and weekly supervisor recorridos need one definition source. | Maintain recorridos as master data/config with role, frequency, owner, and inspection points. | P1 | Pending | Wave 2 partial | Config now defines the inspection-plan structure and shared frequency taxonomy; role-scoped recorrido ownership remains Wave 4. |
| Supervision / Recorridos | Summary clarity | `Abierto`, `Atrasado`, and `Completado` appear to summarize conserje completions, but role scope is unclear. | Clarify whether summaries show all recorridos, only assigned role recorridos, or team oversight metrics. | P1 | Pending | Captured | Supervisor can tell exactly whose recorridos are represented in the summary. |
| Supervision / Recorridos | Continue recorrido ambiguity | `Siguiente paso`, `Recorrido matutino`, and `Continuar puntos` make it unclear whether supervisor is continuing a conserje recorrido or their own recorrido. | Make recorrido ownership and action explicit before continuing. | P1 | Pending | Captured | Supervisor cannot accidentally continue or complete the wrong person's recorrido. |
| Supervision / Recorridos | Point action UX | Clicking points changes a pill to `No aplica`, but the interaction is not intuitive. | Make point status actions clearer, with obvious selected state and meaning. | P2 | Pending | Captured | Supervisor understands what each tap does before completing a recorrido. |
| Supervision / Recorridos | Repeated active execution | `Ejecucion en curso` repeats `Recorrido matutino`, making the screen feel duplicated. | Consolidate repeated recorrido information into one clear active execution area. | P2 | Pending | Captured | Active recorrido information appears once, with clear status and next action. |
| Supervision / Recorridos | Completion end state | Completing the recorrido only changes status to `Completado` with no useful follow-up. | Add a clearer completion state, confirmation, or return path after completion. | P2 | Pending | Captured | After completion, supervisor sees what happened and what to do next. |
| Supervision / Mapa | Map usefulness | Site map still shows no useful operational information. | Apply the same marker/pin/location fixes captured for the map workflow. | P1 | Pending | Captured | Supervisor map has useful operational markers and issue context. |
| Supervision / Incidencias | Incident detail access | Incidents are visible but cannot be opened for useful review. | Allow supervisor to open incident details. | P1 | Pending | Captured | Supervisor can inspect an incident before acting. |
| Supervision / Incidencias | Incident comments/actions | Supervisor cannot add comments, send back, or meaningfully manage incidents. | Add incident action workflow with comments, send-back/reassign where appropriate, and evidence/closure notes. | P1 | Pending | Captured | Supervisor can manage incident lifecycle with visible decision history. |
| Supervision / Incidencias | Resolver action | `Tomar accion` > `Resolver` only changes status without comments. | Require or allow closure comments/evidence when resolving. | P1 | Pending | Captured | Reporter, junta, and management can review why/how an incident was closed. |
| Supervision / Projects and Actions | Section naming | Section is labeled `Project and Action`, mixing languages and concepts. | Rename/clarify this section around work orders/actions/projects according to actual operational use. | P2 | Pending | Captured | Users understand whether they are managing projects, work orders, or action items. |
...
| Supervision / New work order | Assigned-to field | `Asignado` is free text, but should route work to actual users/roles. | Make assignee a dropdown based on configured administration, concierge/operator, and junta users. | P1 | Pending | Wave 2 partial | Work-order assignee uses a controlled dropdown derived from role config, with helper support for admin_users rows when available; real per-user routing remains a later data/API pass. |
...
| Supervision / New work order | Supabase 400 error | Creating a work assignment errors with `Supabase 400 on work assignment`. | Debug and fix work assignment creation, likely tied to required controlled fields/foreign keys. | P1 | Pending | Captured | New work order saves successfully and appears for the assigned user. |
| Supervision / Projects and Actions | Existing project updates | No clear way to update existing projects/actions. | Add or expose update flow for existing work orders/projects/actions. | P1 | Pending | Captured | Supervisor can update existing items, not only create new ones. |
...
| Supervision / Inventario | Role mismatch | Supervisor sees `Registrar conteo`, but count registration belongs to concierge. | Remove/hide count registration from supervisor inventory view. | P1 | Pending | Captured | Supervisor inventory view does not show concierge-only count actions. |
| Supervision / Inventario | Reportar novedad | `Reportar novedad` opens the same fields and does not match supervisor responsibilities. | Replace supervisor inventory actions with purchase-order/status/comment workflows for stockouts and low-stock items. | P1 | Pending | Captured | Supervisor can update procurement/status response to flagged stock issues. |
| Supervision / Inventario | Stockout actioning | Supervisor should be able to act on low stock/stockout flags. | Add actions for purchase order raised, comments, status updates, and follow-up ownership. | P1 | Pending | Captured | Low stock/stockout items can be tracked through supervisor action and comments. |
| Gerencia | Role scope | Gerencia role needs visibility across multiple buildings, supervisors, administradoras de planta, and staff. | Treat Gerencia as a macro oversight role, not a local execution role. | P1 | Pending | Captured | Gerencia screens show portfolio/building-level oversight and drill-down, not local task entry by default. |
| Gerencia / Home | KPI drill-down | KPIs such as `Pendiente de aprobacion`, `Trabajos abiertos`, and `Resumen semanal` cannot be drilled down. | Add drill-down from each KPI into the relevant filtered detail view. | P1 | Pending | Captured | Macro KPIs open useful supporting detail by building/status/owner. |
| Gerencia / Home | Suggested action | `Accion sugerida` says `Abrir reportes`, but the action is basic and not very descriptive. | Improve suggested actions so they guide macro-level oversight decisions. | P2 | Pending | Captured | Suggested action points Gerencia to a useful next review/action. |
...
| Gerencia / Proyectos | Project/action structure | Projects/actions can be raised but lack dropdowns/structure, preventing meaningful progress reporting. | Add structured project/action fields, controlled dropdowns, and progress update model. | P1 | Pending | Wave 2 partial | New project/work-order forms now use controlled area, assignee, and priority dropdowns; progress update model remains later scope. |
| Gerencia / Proyectos | Progress reporting | No clear way to add progress against projects over time. | Add progress/status update workflow for projects/actions. | P1 | Pending | Captured | Project progress is visible and reportable over time. |
| Gerencia / Incidencias | Incident action | Incidents show `Tomar` and `Resolver`, but resolution still lacks comments/context. | Reuse fixed incident action workflow with comments, evidence, escalation, and closure history. | P1 | Pending | Captured | Gerencia can resolve/escalate with reviewable history. |
| Gerencia / Incidencias | Escalation | Gerencia needs a way to escalate incidents, with downstream visibility for Junta. | Ensure escalated incidents become visible/actionable to Junta. | P1 | Pending | Captured | Escalating incident 502831 or similar sends it to Junta view with context. |
| Gerencia / Mapa | Building issue visibility | Map has no visual representation of issues for the specific building. | Show building-specific issue markers and relevant operational map context. | P1 | Pending | Captured | Gerencia can see issues visually on the selected building map. |
| Gerencia / Recorridos | Master plan ownership | `Nuevo plan maestro` appears in Gerencia, but master plan creation should live with the supervisor/admin operational row. | Move or restrict master plan creation to the appropriate operational role, likely supervisor/admin, while Gerencia retains oversight. | P1 | Pending | Captured | Gerencia does not maintain local plan setup unless explicitly intended; correct role owns plan creation. |
| Gerencia / Recorridos | Execution class duplication | `Clases de ejecucion` / recorridos are too fragmented; matutino/diario overlap. | Consolidate execution class/frequency concepts, e.g. use `diario` instead of separate matutino/diario where appropriate. | P2 | Pending | Captured | Plan frequency/classification is simple and not duplicated. |
| Gerencia / Recorridos | Plan area definition | `Area base del plan` is too limited for plan setup. | Plan maestro should define multiple location points, inspection sequence, frequency, and optional comments. | P1 | Pending | Captured | Master plan captures locations, ordered points, frequency, and notes as the source of truth. |
| Junta / Home | KPI wording | Home KPIs show `modulos`, which does not feel like board-level information. | Replace module-oriented KPI wording with governance/oversight metrics that matter to Junta. | P2 | Pending | Captured | Junta dashboard shows board-relevant KPIs, not internal module counts. |
...
| Junta / Home | KPI drill-down | Junta KPIs cannot be drilled down. | Add drill-down from KPI cards into filtered supporting detail. | P1 | Pending | Captured | Junta can open KPI details and see what drives the number/status. |
...
| Junta / Proyectos | Backlog/project visibility | `Backlog`/similar labels appear, but no operational backlog or projects are visible. | Build a clear board-level projects/backlog view once source data exists. | P1 | Pending | Captured | Junta can see capital projects and operational backlog items. |
| Junta / Proyectos | Capital projects | Junta needs top-down view of capital projects. | Show capital projects with milestones, percent completion, RAG status, and supervisor/administration progress comments. | P1 | Pending | Captured | Board can review capital project status and supporting updates. |
| Junta / Proyectos | Operational backlog | Junta needs separate operational backlog for actions owned by supervisors, administration, or Gerencia. | Show operational backlog with owner, status, priority, due date, comments, and escalation state. | P1 | Pending | Captured | Board can see operational actions and who is accountable. |
...
| Junta / Reportes | Suggested actions source | `Acciones sugeridas` appear, but source/logic is unclear. | Clarify or document how suggested actions are generated. | P2 | Pending | Captured | Junta can trust suggested actions because their source is understandable. |
...
| Junta / Escalaciones | Missing escalation | Escalation raised from Gerencia/supervision test did not appear in Junta escalations. | Fix escalation routing so items escalated from administration/Gerencia/residents/concierge-originated incidents appear for Junta. | P1 | Pending | Captured | Escalated incident 502831 or similar appears in Junta escalations with context. |
| Junta / Escalaciones | Escalation source model | Escalations should come from administration, resident-originated issues, concierge-detected issues, or Junta-flagged items. | Define one escalation lifecycle and source model consumed by all roles. | P1 | Pending | Wave 2 partial | Shared escalation status/target taxonomy is defined in config; full cross-role event routing remains Wave 3/Junta workflow scope. |
| Junta / Junta tab | Loading state | Junta tab initially says `Cargando`; eventually shows good KPIs. | Keep loading behavior if short, but ensure it resolves reliably and does not mask errors. | P2 | Pending | Captured | Junta tab loads KPIs reliably with clear fallback if data fails. |
